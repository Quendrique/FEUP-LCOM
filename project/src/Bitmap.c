...
void drawBitmap(Bitmap* bmp, int x, int y, Alignment alignment) {
    ...
    char * bufferStartPos = (char*) malloc(sizeof(char));
    char* imgStartPos;
...
void draw_bitmap_to_buffer(Bitmap* bmp, int x, int y, Alignment alignment, char* buffer) {
    ...
    for (i = 0; i < height; i++) {
        ...
        //memcpy(bufferStartPos, imgStartPos, drawWidth * 2);
        int k;
                for(k = 0; k < drawWidth * 2; k+=2)
                {
                	if(imgStartPos[k] != 0x1f && imgStartPos[k+1] != 0xfffffff8)
                	{
                		bufferStartPos[k] = imgStartPos[k];
                		bufferStartPos[k+1] = imgStartPos[k+1];
                	}
                }
    }
